...
async function createPasskeyNative(username: string): Promise<PasskeyCredential> {
  ...
  try {
    ...
    // Compress the public key if it's uncompressed (65 bytes)
    ...
    if (publicKeyBytes.length === 65 && publicKeyBytes[0] === 0x04) {
      ...
      throw new Error(`Unexpected public key length: ${publicKeyBytes.length}`);
    }
...
async function signWithPasskeyNative(
  ...
  // Convert message to base64url for the challenge
  const msgHashBase64 = base64urlEncode(message);
