...
export class ProgramClient<I extends Idl> {
  ...
  >(
    ...
    return acc
      ? ({ publicKey: typeof pda === 'string' ? pda : pda.toBase58(), ...parser(acc as IdlAccounts<I>[T]) } as R)
      : null;
  }
  ...
  >(
    ...
    return accs.map((acc, i) => {
      return acc
        ? ({ publicKey: typeof pdas[i] === 'string' ? pdas[i] : pdas[i].toBase58(), ...parser(acc as IdlAccounts<I>[T]) } as R)
        : null;
    });
  }
